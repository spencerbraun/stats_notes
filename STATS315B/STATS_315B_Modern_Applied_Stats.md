...
* The derivative of the loss wrt its prediction is called the pseudo response or **generalized residual**: $l_{i}=-\frac{\partial L}{\partial F_{i}}\left(y_{i}, F_{i}\right)$. On the path, this becomes $l_{i}(\nu)=-\frac{\partial L}{\partial F_{i}(\nu)}\left(y_{i}, F_{i}(\nu)\right)$. This quantity is fundamentally important to boosting.
* Generalized residual: our ultimate loss is least-squares. For the squared error loss the generalized residual is the ordinary residual. For absolute value loss, we get the sign of the residual for the generalized residual. For any differentiable loss, we can get some generalized residual. 
...
* Lasso Gradient Boosting
  ...
  * We don't both checking for lambdas opposite the sign of the coefficient. This simplification just means we won't track non-monotone paths as closely, but large efficiency increase.
...
* Compute the generalized residuals at each point. Use tree building routine with GR as outcome  and it returns a region set. Decide on step size - finding the optimal coefficient by which to add our tree, multiplied by some $\varepsilon$, then add our tree to the model. This changes the GR and we repeat. 