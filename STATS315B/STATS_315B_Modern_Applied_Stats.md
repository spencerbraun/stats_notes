...
* Structural Model: $F(x)=\sum_{m=1}^{M} b_{m} S\left(a_{0 m}+\sum_{j=1}^{m} a_{j m} x_{j}\right)+b_{0}$
	...
	* For M arbitrarily large, we can approximate any function. But of course for M large, we might be better off using another method. Trees, boosting can also approximate any function.
...
* Score $Q(w) = \frac{1}{N}\sum_{i=1}^N[y_i - F(x_i;w))]^2$
* Take gradient $g_k = \frac{\partial Q(w)}{w_k}$
* Then gradient descent:
	* Take initial guess $w_0$
	* Loop $w \leftarrow w - \epsilon g(w)$ until $|g(W)| = 0$
